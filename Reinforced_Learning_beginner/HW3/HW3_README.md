...
- **TAU** - коэффициент для обновления обновления target сети (в коде critic target). Во всех экспериментах советуют брать величину, равную 0,005. Слишком большой коэффициент может привести к нестабильной работе агента, слишком маленький – замедлить обучение. На рисунке ниже исследование, проведенное авторами статьи «Soft Actor-Critic: Off-Policy Maximum Entropy Deep Reinforcement. Learning with a Stochastic Actor» (https://arxiv.org/pdf/1801.01290.pdf).
  ...
  ![image](https://github.com/shakhovak/Study-projects-in-Uni/assets/89096305/062e2d1f-5ff6-4904-84a7-0fff110a286e)
...
### Эксперимент 3 - детерминированная политика без эноропии.
В данном эксперименте не используется дополнительная максимизация энтропии, поэтому температура равна нулю.
В этом ноутбуке ([ссылка](https://www.kaggle.com/code/katyashakhova/exp3-cheetah-sac-train)) обучение для **эксперимента 3**. Результаты обучения логируются в wandb и их можно посмотреть вот [здесь](https://wandb.ai/shakhova/RL_SAC?nw=nwuserkatya_shakhova). В этом же ноутбуке есть небольшой ролик, который показывает работу агента после обучения. Ниже приведен принт-скрин из wandb по обучению модели.

![image](https://github.com/shakhovak/Study-projects-in-Uni/assets/89096305/eb606d4f-0f36-4ad5-a1d1-1f785aa93a25)

Из этого графика видно, что обучение идет гораздо медленнее, чем с энтропией. Также есть резкие падения вознаграждения даже в конце обучения. Данный график подтверждает выводы авторов статей, что при отсутсвии максимизации энтропии нужно больше времени на обучение.
Интересно посмотреть и на динамику функции потерь:

![image](https://github.com/shakhovak/Study-projects-in-Uni/assets/89096305/bf07e7d3-cef6-413c-8ea3-b1e83d610d28)

Результаты не сильно отличаются от предыдущих экспериментов, по сравнению с лучшим вариантом при максимизации энтропии потери немного больше.

Веса модели для этого эксперимента сохранены как output в самом ноутбуке. Инференс по этому эксперименту не делала, так как результаты немного хуже, чем по эксперименту 1.

## Финальные выводы.
Алгоритм SAC с использованием максимизации энтропии при использовании оптимального коэффициента тепмературы показывает лучшие результаты, чем просто обучение на максимизацию вознаграждения. Использование автоподбора коэффициента позволит быстро достичь оптимальных результатов.
